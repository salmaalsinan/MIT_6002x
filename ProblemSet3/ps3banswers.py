...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 16 00:23:55 2019
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        """
        reproductionProb = self.maxBirthProb * (1 - popDensity)
        resToAllDrugs = all(self.isResistantTo(drug) for drug in activeDrugs)

        if resToAllDrugs and random.random() <= reproductionProb:
            newResistances = {}

            for drug, res in self.resistances.items():
                if random.random() > self.mutProb:
                    newResistances[drug] = res
                else:
                    newResistances[drug] = not res
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        """
        if not newDrug in self.administeredDrugs:
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.plot(resultsTotalAvg, label="total virus population")
    pylab.plot(resultsGtResAvg, label="guttagonol-resistant virus population")
    pylab.xlabel("timesteps")
    pylab.ylabel("average virus population")
    pylab.title("Simple Simulation (With Drug Treatment)")
    pylab.legend()
    pylab.show()
